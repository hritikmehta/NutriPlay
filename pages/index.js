...
export default function Home() {
  ...
  if (gameState === GAME_STATES.GAME_OVER) {
    ...
    const APP_URL = 'https://nutri-play.vercel.app'
    const categoryNames = selectedCategories.includes('random')
      ? 'Random Mix'
      : selectedCategories.map(id => CATEGORIES.find(c => c.id === id)?.name || id).join(', ')
    const shareText = `I just played NutriPlay 🥕 — a fun nutrition trivia game!\n\nCategory: ${categoryNames}\nScore: ${correctAnswers} correct out of ${attempts} attempts (${accuracy}% accuracy)\n\nThink you can beat me? Give it a try 👇\n${APP_URL}`
    ...
    const handleNativeShare = async () => {
      if (navigator.share) {
        try {
          await navigator.share({ title: 'NutriPlay', text: shareText, url: APP_URL })
        } catch (e) {
    ...
    return (
      <div className="min-h-screen bg-orangePrimary flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8 text-center">
          ...
          {/* Share CTA */}
          <div className="mb-5">
            <p className="text-sm font-semibold text-gray-700 mb-2">Challenge your friends! 🎯</p>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-gray-700 hover:bg-gray-800 text-white px-4 py-2.5 rounded-lg font-semibold text-sm transition-colors"
              >
                <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current" xmlns="http://www.w3.org/2000/svg">
