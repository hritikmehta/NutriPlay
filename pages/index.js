...
export default function Home() {
  ...
  const buildShareMessage = (correct, total, acc) => {
    const siteUrl = typeof window !== 'undefined' ? window.location.origin : 'https://nutriplay.app'
    return `I just scored ${correct}/${total} on NutriPlay! 🥕\nAccuracy: ${acc}%\n\nCan you beat me? Test your nutrition knowledge 👇\n${siteUrl}`
  }

  const handleShare = (correct, total, acc) => {
    const message = buildShareMessage(correct, total, acc)
    const siteUrl = typeof window !== 'undefined' ? window.location.origin : 'https://nutriplay.app'
    if (navigator.share) {
      navigator.share({ title: 'NutriPlay', text: message, url: siteUrl }).catch(() => {})
    } else {
  ...
  const handleCopyLink = (correct, total, acc) => {
    const message = buildShareMessage(correct, total, acc)
    ...
    return (
      <div className="min-h-screen bg-orangePrimary flex items-center justify-center p-4">
        <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8 text-center">
          ...
          {/* Share CTA */}
          <div className="mb-6">
            <p className="text-xs text-gray-400 mb-2 uppercase tracking-wide font-semibold">Challenge your friends</p>
            <div className="flex gap-2">
              <button
                onClick={() => handleShare(correctAnswers, attempts, accuracy)}
                className="flex-1 flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white px-4 py-3 rounded-lg font-semibold transition-colors text-sm"
              >
                <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current flex-shrink-0" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M12 0C5.373 0 0 5.373 0 12c0 2.128.558 4.121 1.531 5.847L.057 23.5l5.817-1.452A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.891 0-3.666-.506-5.197-1.388l-.373-.22-3.453.862.92-3.349-.243-.386A9.944 9.944 0 012 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/>
                </svg>
                {typeof window !== 'undefined' && navigator.share ? 'Share Results' : 'Share on WhatsApp'}
              </button>
              <button
                onClick={() => handleCopyLink(correctAnswers, attempts, accuracy)}
                className={`flex items-center justify-center gap-2 px-4 py-3 rounded-lg font-semibold transition-colors text-sm border ${copyLinkSuccess ? 'bg-green-50 border-green-400 text-green-600' : 'bg-gray-50 border-gray-200 text-gray-600 hover:bg-gray-100'}`}
              >
                {copyLinkSuccess ? (
                  <>
